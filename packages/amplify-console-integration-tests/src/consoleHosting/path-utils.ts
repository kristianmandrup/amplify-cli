...
export const amplifyRootPathFor = (root: string, ...paths: any[]) => {
  return path.join(root, $paths.amplifyPath, ...paths);
};
...
export const backendRootPathFor = (root: string, ...paths: any[]) => {
  return amplifyRootPathFor(root, $paths.backendPath, ...paths);
};
